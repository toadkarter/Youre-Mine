...
var original_horizontal_position: float
...
func _ready() -> void:
	...
	original_horizontal_position = position.x
	gravity_scale = 0.0
	hitbox.connect("body_entered", _on_body_entered)


func _physics_process(_delta: float) -> void:
	if position.x != original_horizontal_position:
		position.x = original_horizontal_position
